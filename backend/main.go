...
// ---------- Structs matching the Rust frontend types ----------
...
type HourlyData struct {
	...
	Pressure       float64  `json:"pressure"`
	SeaTemperature *float64 `json:"sea_temperature,omitempty"`
	UVIndex        float64  `json:"uv_index"` // new
}
...
type OpenMeteoForecast struct {
	Current struct {
		...
		SeaSurfaceTemperature *float64 `json:"sea_surface_temperature,omitempty"`
		UVIndex              float64  `json:"uv_index"` // new
	} `json:"current"`
	Hourly struct {
		Time                  []string  `json:"time"`
	...
	Daily struct {
		...
		UVIndexMax            []float64 `json:"uv_index_max"` // new
	} `json:"daily"`
}
...
// ---------- Yandex Cloud Function event types ----------
...
// ---------- Forecast and Yesterday ----------

func fetchForecast(lat, lon float64) (*OpenMeteoForecast, error) {
	url := fmt.Sprintf(
		"https://api.open-meteo.com/v1/forecast?latitude=%.4f&longitude=%.4f"+
			"&current=temperature_2m,wind_speed_10m,weather_code,relative_humidity_2m,surface_pressure,sea_surface_temperature,uv_index"+
			"&hourly=temperature_2m,wind_speed_10m,weather_code,relative_humidity_2m,surface_pressure,sea_surface_temperature,uv_index"+
			"&daily=temperature_2m_max,temperature_2m_min,wind_speed_10m_max,weather_code,sunrise,sunset,relative_humidity_2m_max,relative_humidity_2m_min,uv_index_max"+
			"&timezone=auto",
		lat, lon,
	)
...
func getWeatherData(city string) (WeatherResponse, error) {
	...
	if fRes.err != nil {
		return WeatherResponse{}, fmt.Errorf("forecast error: %w", fRes.err)
	}
	if yRes.err != nil {
		return WeatherResponse{}, fmt.Errorf("yesterday error: %w", yRes.err)
	}
	...
	current := CurrentData{
		...
		SeaTemperature: forecast.Current.SeaSurfaceTemperature,
	...
	if len(forecast.Hourly.Time) > 0 {
		...
		for i, isoTime := range forecast.Hourly.Time {
			...
			var seaTemp *float64
			if len(forecast.Hourly.SeaSurfaceTemperature) > i {
				seaTemp = forecast.Hourly.SeaSurfaceTemperature[i]
			}
			hourly = append(hourly, HourlyData{
				...
				Pressure:       forecast.Hourly.SurfacePressure[i],
	...
	for i, date := range forecast.Daily.Time {
		...
		forecastDays = append(forecastDays, DailyData{
			...
			MoonPhaseName:    &moonName,
			MoonIllumination: &moonIllum,
			HumidityMax:      forecast.Daily.RelativeHumidity2mMax[i],
			HumidityMin:      forecast.Daily.RelativeHumidity2mMin[i],
...
func Handler(w http.ResponseWriter, r *http.Request) {
	...
	if city == "" {
		...
		json.NewEncoder(w).Encode(map[string]string{"error": "Missing 'city' query parameter"})
		return
	}
